...
<main class="container-fluid py-5 mb-5">
    <div class="px-3 mb-5">
        <div class="d-flex justify-content-between align-items-center w-100 mb-5">
            <h5 class="card-title m-0">재테크 월렛 출금하기</h5>
            <div class="m-0">
                <a href="{{ route('wallet.withdrawal.list') }}">
                    <h5 class="btn btn-outline-primary m-0">출금내역</h5>
                </a>
            </div>
        </div>
        <form method="POST" action="{{ route('wallet.withdrawal.store') }}" id="withdrawalForm">
            @csrf
            <fieldset>
                <legend class="fs-3 text-dark mb-3">출금할 가상자산을 선택 해주세요.</legend>
                    <div class="d-grid d-grid-col-2 mb-3">
                        ...
                        <div class="selectedWallet">
                            ...
                            <input type="hidden" value="{{ $wallet->withdrawal_fee_rate }}" class="withdrawal_fee_rate">
                        </div>
                        @endforeach
                    </div>                
            </fieldset>
            <div class="mt-4 mb-3">
                <label class="form-label">출금 수량을 입력하세요.</label>
                <input type="text" name="amount" id="amount" class="form-control" placeholder="0">
            </div>
            <p id="feeString" class="mb-5"></p>
            <button type="submit" class="btn btn-primary w-100 py-3 mb-4 fs-4">출금하기</button>                                
        </form>
    </div>
</main>
...
@push('script')
<script src="{{ asset('js/wallet/withdrawal.js') }}"></script>
@endpush
