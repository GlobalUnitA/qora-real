$(document).ready(function() {
    ...
    $("input[name='wallet']").change(function () {
        ...
        const deposit_fee_rate = parseFloat(seletedWallet.closest('.selectedWallet').find('.deposit_fee_rate').val()).toFixed(0);

        $('#feeString').html(`입금 수수료 ${deposit_fee_rate}%이며, 입금 시 수수료 차감 후 재테크 월렛에 반영 됩니다.`);
    ...
    $('#depositForm').submit(function (event) {
        ...
        if (!wallet) {
            alertModal('입금할 가상자산을 선택하세요.');
            return;
        }

        if (!amount) {
            alertModal('입금 수량을 입력하세요.');
            return;
        }
